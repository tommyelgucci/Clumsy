...
/** Rounds an angle (radians) to the nearest multiple of `step`. */
export const snapAngle = (angle: number, step: number) => Math.round(angle / step) * step;

/* ------------------------------------------------------------------ *
 * 3x3 affine matrix, column-major, ready for `uniformMatrix3fv`.
 * ------------------------------------------------------------------ */
...
export function mat3FromTRS(
  ...
  const d = -s * scaleY;
  const e = c * scaleY;
  // Translate to the origin, scale+rotate, and back, all folded together.
...
/* ------------------------------------------------------------------ *
 * One Euro filter: smooths a stroke without adding the constant lag a
 * moving average introduces. See Casiez et al., CHI 2012.
 * ------------------------------------------------------------------ */
